...
{
  uint32_t newMask = 0;
  const bool frameFresh = _model.state.input.frameTime != 0 &&
                          (micros() - _model.state.input.frameTime) < 150000;
  const bool rxValid = _model.state.input.frameCount >= 5 &&
                       frameFresh &&
  ...
  for(size_t i = 0; i < ACTUATOR_CONDITIONS; i++)
  {
    ...
    if(val > min && val < max)
    {
      // Prevent false activation from default/stale AUX values before RX is valid.
      if(c->id == MODE_GPIO_OUTPUT && !rxValid) continue;
      newMask |= 1 << c->id;
    }
  }
...
void Actuator::updateGpioOutput()
{
  static uint8_t gpioPin = 5; // GPIO pin 5 for auxiliary output
  ...
  const bool rxValid = _model.state.input.frameCount >= 5 &&
                       ...
                       !_model.state.input.rxLoss &&
                       !_model.state.input.rxFailSafe;
  const bool gpioOutputActive = rxValid && _model.isModeActive(MODE_GPIO_OUTPUT);

  // Initialize the pin on first call
  if(!initialized)
  {
    Hal::Gpio::pinMode(gpioPin, OUTPUT);
    // Set a deterministic startup level even when mode did not change.
    Hal::Gpio::digitalWrite(gpioPin, gpioOutputActive ? HIGH : LOW);
    initialized = true;
  }

  if(!rxValid)
  {
    Hal::Gpio::digitalWrite(gpioPin, LOW);
    return;
  }

  // Check if MODE_GPIO_OUTPUT mode has changed
  if(_model.hasChanged(MODE_GPIO_OUTPUT))
  {
    if(gpioOutputActive)
    {
      // Mode activated: set pin to 3.3V (HIGH)
      Hal::Gpio::digitalWrite(gpioPin, HIGH);
    }
    else
    {
      // Mode deactivated: set pin to 0V (LOW)
      Hal::Gpio::digitalWrite(gpioPin, LOW);
    }
  }
}
