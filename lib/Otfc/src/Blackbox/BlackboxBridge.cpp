...
static Otfc::Model * _model_ptr = nullptr;

void initBlackboxModel(Otfc::Model * m)
{
  _model_ptr = m;
}
...
{
  return Otfc::Utils::toDeg(_model_ptr->state.setpoint.rate[axis]);
}

float mixerGetThrottle(void)
{
  return (_model_ptr->state.output.ch[Otfc::AXIS_THRUST] + 1.0f) * 0.5f;
}
