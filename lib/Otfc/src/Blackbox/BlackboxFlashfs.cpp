...
using BufferType = Otfc::Utils::RingBuf<uint8_t, FLASHFS_WRITE_BUFFER_SIZE>;
...
{
    flashfs.partition = Otfc::Device::FlashDevice::findPartition();
...
bool IRAM_ATTR flashfsFlushAsync(bool force)
{
    ...
    {
        //uint32_t newAddress = force ? (flashfs.address + size) : Otfc::Utils::alignAddressToWrite(flashfs.address, size, FLASHFS_FLUSH_BUFFER_SIZE);
