...
const Cli::Param * Cli::initialize(ModelConfig& c)
{
  ...
  static const char* altHoldThrottleChoices[] = { PSTR("STICK"), PSTR("MID_STICK"), PSTR("HOVER"), NULL };
  ...
  static const Param params[] = {
    ...
    Param(PSTR("rangefinder_bus"), &c.rangefinder.bus, busDevChoices),
    Param(PSTR("rangefinder_dev"), &c.rangefinder.dev, rangefinderDevChoices),
    Param(PSTR("rangefinder_max_range"), &c.rangefinder.maxRange),
    Param(PSTR("rangefinder_lpf_type"), &c.rangefinder.filter.type, filterTypeChoices),
    Param(PSTR("rangefinder_lpf_freq"), &c.rangefinder.filter.freq),
    ...
    Param(PSTR("pid_mag_p"), &c.pid[FC_PID_MAG].P),

    Param(PSTR("pid_althold_pos_p"), &c.pid[FC_PID_ALT].P),
    Param(PSTR("pid_althold_pos_i"), &c.pid[FC_PID_ALT].I),
    Param(PSTR("pid_althold_pos_d"), &c.pid[FC_PID_ALT].D),
    Param(PSTR("pid_althold_pos_f"), &c.pid[FC_PID_ALT].F),
    ...
    Param(PSTR("pid_althold_baro_v_weight"), &c.altHold.baroVarioWeight),
    Param(PSTR("pid_althold_surface_weight"), &c.altHold.surfaceWeight),
    Param(PSTR("pid_althold_acc_bias_ground"), reinterpret_cast<int16_t*>(&c.altHold.accelBiasAlphaGround)),
    Param(PSTR("pid_althold_acc_bias_air"), reinterpret_cast<int16_t*>(&c.altHold.accelBiasAlphaAir)),

    Param(PSTR("pid_poshold_pos_p"), &c.pid[FC_PID_POS].P),
    Param(PSTR("pid_poshold_pos_i"), &c.pid[FC_PID_POS].I),
    Param(PSTR("pid_poshold_vel_p"), &c.pid[FC_PID_POSR].P),
    Param(PSTR("pid_poshold_vel_i"), &c.pid[FC_PID_POSR].I),
    Param(PSTR("pid_poshold_vel_d"), &c.pid[FC_PID_POSR].D),
    Param(PSTR("poshold_flow_gain_x"), &c.posHold.flowGainX),
    Param(PSTR("poshold_flow_gain_y"), &c.posHold.flowGainY),
    Param(PSTR("poshold_deadband"), &c.posHold.deadband),
    Param(PSTR("poshold_angle_limit"), &c.posHold.angleLimit),
    Param(PSTR("poshold_use_gyro_comp"), &c.posHold.useGyroComp),
    Param(PSTR("poshold_vel_lpf_type"), &c.posHold.velFilter.type, filterTypeChoices),
    Param(PSTR("poshold_vel_lpf_freq"), &c.posHold.velFilter.freq),
...
bool Cli::process(const char c, CliCmd& cmd, Stream& stream)
{
  ...
  // While flow watch is active, collect user input but do not execute commands.
...
void Cli::tick(Stream& stream)
{
  ...
  stream.print(F(" v="));
  stream.print(f.valid ? 1 : 0);
  stream.print(F(" n="));
  stream.print(f.updateCount);
  stream.print(F(" mx="));
  stream.print(f.motionX);
  stream.print(F(" my="));
  stream.println(f.motionY);
}
...
void Cli::execute(CliCmd& cmd, Stream& s)
{
...
#if defined(OTFC_WIFI) || defined(OTFC_WIFI_ALT)
  ...
  {
    if(cmd.args[1] && strcmp_P(cmd.args[1], PSTR("watch")) == 0)
    {
      ...
      int32_t periodMs = _flowWatchPeriodMs;
      if(cmd.args[2])
      {
        const int32_t reqMs = static_cast<int32_t>(String(cmd.args[2]).toInt());
        periodMs = (reqMs < 10) ? 10 : reqMs;
      }
...
void Cli::printRangefinderStatus(Stream& s) const
{
  s.println(F("RANGEFINDER STATUS:"));

  s.print(F("  Present: "));
  s.println(_model.state.rangefinder.present ? F("Yes") : F("No"));

  if(_model.state.rangefinder.present)
  {
    s.print(F("  Device: "));
    if(_model.state.rangefinder.dev)
    {
      s.println(FPSTR(Device::RangefinderDevice::getName(_model.state.rangefinder.dev->getType())));
    }
    else
    {
      s.println(FPSTR(Device::RangefinderDevice::getName((RangefinderDeviceType)_model.config.rangefinder.dev)));
    }

    s.print(F("  Rate: "));
    s.print(_model.state.rangefinder.rate);
    s.println(F(" Hz"));

    s.print(F("  Samples: "));
    s.println(_model.state.rangefinder.updateCount);

    s.print(F("  Valid: "));
    s.println(_model.state.rangefinder.valid ? F("Yes") : F("No"));

    s.print(F("  Raw: "));
    s.print(_model.state.rangefinder.raw);
    s.println(F(" mm"));

    s.print(F("  Distance: "));
    s.print(_model.state.rangefinder.distance, 3);
    s.println(F(" m"));
...
{
  s.println(F("OPTIC FLOW STATUS:"));

  s.print(F("  Present: "));
  s.println(_model.state.flow.present ? F("Yes") : F("No"));

  if(_model.state.flow.present)
  {
    s.print(F("  Valid: "));
    s.println(_model.state.flow.valid ? F("Yes") : F("No"));

    s.print(F("  Quality: "));
    s.println(_model.state.flow.quality);

    s.print(F("  Samples: "));
    s.println(_model.state.flow.updateCount);

    s.print(F("  Motion X: "));
    s.println(_model.state.flow.motionX);

    s.print(F("  Motion Y: "));
    s.println(_model.state.flow.motionY);
  }
}
