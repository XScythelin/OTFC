...
#include <algorithm>
#include <limits>
#if defined(OTFC_MULTI_CORE) && defined(OTFC_FREE_RTOS)
...
static Otfc::SerialSpeed fromBaudIndex(SerialSpeedIndex index)
{
  using namespace Otfc;
  switch(index)
  {
...
{
  switch(t) {
    case 0: return Otfc::FILTER_NONE;
    case 1: return Otfc::FILTER_PT3;
    case 2: return Otfc::FILTER_BIQUAD;
    default: return Otfc::FILTER_PT3;
  }
}
...
{
  switch(t) {
    case Otfc::FILTER_NONE: return 0;
    case Otfc::FILTER_PT3: return 1;
    case Otfc::FILTER_BIQUAD: return 2;
    default: return 1;
  }
}
...
{
  switch(t) {
    case Otfc::GYRO_DLPF_256: return 0;
    case Otfc::GYRO_DLPF_EX: return 1;
    default: return 2;
  }
}
...
// Betaflight MSP2 sensor feed messages (sent by the sensor, e.g. MTF-02, to the FC)
constexpr uint16_t MSP2_SENSOR_RANGEFINDER = 0x1F01;
constexpr uint16_t MSP2_SENSOR_OPTIC_FLOW  = 0x1F02;

// OTFC custom read commands, let an external host poll the latest sensor data
constexpr uint16_t MSP2_OTFC_RANGEFINDER  = 0x4F01;
constexpr uint16_t MSP2_OTFC_OPTIC_FLOW   = 0x4F02;
...
void MspProcessor::processCommand(MspMessage& m, MspResponse& r, Device::SerialDevice& s)
{
  ...
  switch(m.cmd)
  {
    ...
    case MSP_BOXNAMES:
      r.writeString(F("ARM;AIRMODE;ANGLE;ALTHOLD;BEEPER;FAILSAFE;BLACKBOX;BLACKBOXERASE;GPIO;MAG;SURFACE;POSHOLD;"));
      break;
    ...
    case MSP_BOXIDS:
      ...
      r.writeU8(MODE_GPIO_OUTPUT);
      r.writeU8(MODE_MAG);
      r.writeU8(MODE_SURFACE);
    ...
    case MSP_SONAR_ALTITUDE:
      // surface (rangefinder) altitude [cm], 0 when no valid reading (too close or tilted)
      r.writeU32(_model.rangefinderActive() && _model.state.rangefinder.valid
                     ? lrintf(_model.state.rangefinder.height * 100.f)
                     : 0);
      break;

    case MSP2_OTFC_RANGEFINDER:
      // full rangefinder readout for an external host
      {
        const Otfc::RangefinderState& rf = _model.state.rangefinder;
        r.writeU8(rf.extQuality);                          // signal quality [0;255] (MSP source)
        r.writeU8(rf.valid ? 1 : 0);                       // valid in-range reading
        r.writeU32((uint32_t)rf.raw);                      // last raw slant distance [mm]
        r.writeU32((uint32_t)lrintf(rf.height * 1000.f));  // tilt-compensated height [mm]
      }
      break;

    case MSP2_OTFC_OPTIC_FLOW:
      // optical flow readout for an external host
      {
        const Otfc::FlowState& f = _model.state.flow;
        r.writeU8(f.quality);          // signal quality [0;255]
        r.writeU8(f.valid ? 1 : 0);    // valid reading
        r.writeU32((uint32_t)f.motionX); // raw motion X
        r.writeU32((uint32_t)f.motionY); // raw motion Y
      }
      break;
    ...
    // --- НОВОЕ: сырые значения с пульта (до override) ---  
    case MSP_RC_RAW:  
      for(size_t i = 0; i < _model.state.input.channelCount; i++)  
      {  
        r.writeU16((uint16_t)_model.state.input.buffer[i]);  
      }  
      break;
    ...
    case MSP_SET_PID:
      {
      const size_t pidPacketItems = m.remain() / 3;
      const size_t pidItemsToRead = std::min((size_t)PID_ITEM_COUNT, pidPacketItems);
      const bool legacyPidPacket = pidPacketItems <= FC_PID_MAG + 1; // legacy payloads tend to clobber ALT/POS fields
      for (size_t i = 0; i < pidItemsToRead; i++)
      {
        const uint8_t p = m.readU8();
        const uint8_t in = m.readU8();
        const uint8_t d = m.readU8();

        if(legacyPidPacket && (i == FC_PID_ALT || i == FC_PID_POS || i == FC_PID_POSR || i == FC_PID_NAVR)) {
          continue;
        }

        _model.config.pid[i].P = p;
        _model.config.pid[i].I = in;
        _model.config.pid[i].D = d;
      }
      }
      _model.reload();
      break;
...
#if defined(OTFC_DEV_PRESET_UNSAFE_ARMING)
        (void)cmd;
#warning "Danger macro used OTFC_DEV_PRESET_UNSAFE_ARMING"
#else
  ...
  case MSP_GPSSVINFO:
    ...
    case MSP_SET_RAW_RC:  
    {  
      size_t channelCount = m.received / 2; // каждый канал — 2 байта (uint16)  
      for (size_t i = 0; i < channelCount && i < INPUT_CHANNELS; i++)  
      {  
        _model.state.input.overrideUs[i] = (float)m.readU16();  
      }  
      _model.state.input.overrideCount = channelCount < INPUT_CHANNELS ? channelCount : INPUT_CHANNELS;  
      _model.state.input.overrideActive = true;  
      _model.state.input.overrideTime = micros();  
    }  
    break;
    ...
    case MSP2_SENSOR_RANGEFINDER:
      // incoming distance feed from an external UART sensor (e.g. MTF-02 lidar)
      {
        ...
        rf.extQuality = quality;
        rf.extRaw = distanceMm;
        rf.extTs = micros();
      }
      r.result = 0; // no reply, this is a push message
      break;
    ...
    case MSP2_SENSOR_OPTIC_FLOW:
      // incoming optical flow feed from an external UART sensor (e.g. MTF-02)
      {
        ...
        Otfc::FlowState& f = _model.state.flow;
        f.quality = quality;
        f.motionX = motionX;
        f.motionY = motionY;
        f.lastMsgTs = micros();
...
{
  // sensor push messages (MTF-02 etc.) are one-way, never acknowledge them
  if(r.cmd == MSP2_SENSOR_RANGEFINDER || r.cmd == MSP2_SENSOR_OPTIC_FLOW) return;
