...
int FAST_CODE_ATTR Fusion::update()
{
  ...
  if(_model.accelActive())
  {
    ...
    {
      _model.state.debug[3] = lrintf((_model.state.accel.world[0] / ACCEL_G) * 2048);
      _model.state.debug[4] = lrintf((_model.state.accel.world[1] / ACCEL_G) * 2048);
      _model.state.debug[5] = lrintf((_model.state.accel.world[2] / ACCEL_G) * 2048);
...
{
  if(_model.magActive() && _model.isModeActive(MODE_MAG))
  {
    _madgwick.update(
