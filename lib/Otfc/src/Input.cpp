...
{  
  ...
  // --- OVERRIDE: проверяем таймаут (500ms без MSP пакетов — отключаем) ---  
  if(_model.state.input.overrideActive &&  
     (micros() - _model.state.input.overrideTime) > 500000)  
  {  
    _model.state.input.overrideActive = false;  
  }  
  ...
  {  
    ...
    // --- OVERRIDE: если активен, подменяем только реально присланные каналы ---  
    if(_model.state.input.overrideActive && c < _model.state.input.overrideCount)  
    {  
      v = _model.state.input.overrideUs[c];  
    }  
