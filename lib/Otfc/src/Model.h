#ifndef _OTFC_MODEL_H_
#define _OTFC_MODEL_H_
...
class Model
{
  public:
    ...
    // altitude hold is engaged by either the plain (baro+accel) or the surface (baro+accel+rangefinder) mode
    bool isAltHoldActive() const
    {
      return isModeActive(MODE_ALTHOLD) || isModeActive(MODE_SURFACE);
    }

    bool isPosHoldActive() const
    {
      return isModeActive(MODE_POSHOLD);
    }
    ...
    {
      return state.rangefinder.present && config.rangefinder.dev != RANGEFINDER_NONE && isFeatureActive(FEATURE_RANGEFINDER);
    }
...
#warning "Danger macro used OTFC_DEV_PRESET_UNSAFE_ARMING"
    ...
    void sanitize()
    {
      ...
      {
        state.gyro.rate = Utils::alignToClock(state.gyro.clock, OTFC_GYRO_SPI_RATE_MAX);
      }
      else
      {
        state.gyro.rate = Utils::alignToClock(state.gyro.clock, OTFC_GYRO_I2C_RATE_MAX);
        // first usage
      ...
      uint32_t featureAllowMask =  FEATURE_RX_PPM | FEATURE_RX_SERIAL | FEATURE_MOTOR_STOP | FEATURE_SOFTSERIAL | FEATURE_GPS |
        FEATURE_TELEMETRY | FEATURE_RX_SPI | FEATURE_RANGEFINDER;// | FEATURE_AIRMODE;
