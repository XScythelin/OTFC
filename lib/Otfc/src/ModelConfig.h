#ifndef _OTFC_MODEL_CONFIG_H_
#define _OTFC_MODEL_CONFIG_H_
...
enum FlightMode {
  ...
  MODE_GPIO_OUTPUT,
  MODE_MAG,
  MODE_SURFACE,  // alt hold using baro + accel + rangefinder
  MODE_POSHOLD,  // optical-flow position hold
};

enum AltHoldThrottleMode {
  ALTHOLD_THROTTLE_STICK,
  ALTHOLD_THROTTLE_MID,
  ALTHOLD_THROTTLE_HOVER,
};
...
enum Feature {
  ...
  FEATURE_GPS        = 1 << 7,
  FEATURE_RANGEFINDER = 1 << 9,  // labelled "Sonar" in Betaflight Configurator
...
static_assert(ESC_CHANNEL_COUNT == OTFC_OUTPUT_COUNT, "ESC_CHANNEL_COUNT and OTFC_OUTPUT_COUNT must be equal");
...
enum PinFunction {
  ...
  PIN_OUTPUT_2,
  PIN_OUTPUT_3,
#if OTFC_OUTPUT_COUNT > 4
  PIN_OUTPUT_4,
#endif
#if OTFC_OUTPUT_COUNT > 5
  PIN_OUTPUT_5,
#endif
#if OTFC_OUTPUT_COUNT > 6
  PIN_OUTPUT_6,
#endif
#if OTFC_OUTPUT_COUNT > 7
  PIN_OUTPUT_7,
#endif
  PIN_BUTTON,
  PIN_BUZZER,
...
struct OutputConfig
{
  ...
  OutputChannelConfig channel[OTFC_OUTPUT_COUNT];
};
...
{
  int8_t bus = BUS_AUTO;
  int8_t dev = RANGEFINDER_VL53L0X;
  int16_t maxRange = 150; // cm, max distance used for altitude fusion
  FilterConfig filter{FILTER_PT1, 10};
};
...
struct AltHoldConfig
{
  ...
  int8_t baroVarioWeight = 20;    // baro vario correction per new baro sample [%]
  int8_t surfaceWeight = 5;       // rangefinder blend in MODE_SURFACE [%]
  int16_t accelBiasAlphaGround = 100; // accel bias correction while grounded [x1e-4]
  int16_t accelBiasAlphaAir = 5;      // accel bias correction in air [x1e-4]
};

struct PosHoldConfig
{
  // raw optical-flow -> ground-velocity gain, x0.0001 [1/raw]. Sign flips per-axis
  // to match sensor orientation (see docs). Multiplied by surface distance.
  int16_t flowGainX = 100;
  int16_t flowGainY = 100;
  int8_t deadband = 5;      // roll/pitch stick deadband [%], inside it position is held
  int8_t angleLimit = 20;   // max auto lean angle while holding [deg]
  bool useGyroComp = true;  // subtract rotation-induced flow using gyro
  FilterConfig velFilter{FILTER_PT1, 5};
};
...
class ModelConfig
{
  public:
    ...
    PidConfig pid[FC_PID_ITEM_COUNT] = {
      ...
      [FC_PID_ALT]   = { .P = 30, .I =  0, .D =  0, .F =  0 },  // ALTHOLD POS
      [FC_PID_POS]   = { .P = 15, .I =  0, .D =  0, .F =  0 },  // POSHOLD position -> velocity
      [FC_PID_POSR]  = { .P = 30, .I = 10, .D =  8, .F =  0 },  // POSHOLD velocity -> lean angle
    ...
    PosHoldConfig posHold;
    ControllerConfig controller;
    // hardware
    int8_t pin[PIN_COUNT] = {
#ifdef OTFC_INPUT
      [PIN_INPUT_RX] = OTFC_INPUT_PIN,
#endif
      [PIN_OUTPUT_0] = OTFC_OUTPUT_0,
      [PIN_OUTPUT_1] = OTFC_OUTPUT_1,
      [PIN_OUTPUT_2] = OTFC_OUTPUT_2,
      [PIN_OUTPUT_3] = OTFC_OUTPUT_3,
#if OTFC_OUTPUT_COUNT > 4
      [PIN_OUTPUT_4] = OTFC_OUTPUT_4,
#endif
#if OTFC_OUTPUT_COUNT > 5
      [PIN_OUTPUT_5] = OTFC_OUTPUT_5,
#endif
#if OTFC_OUTPUT_COUNT > 6
      [PIN_OUTPUT_6] = OTFC_OUTPUT_6,
#endif
#if OTFC_OUTPUT_COUNT > 7
      [PIN_OUTPUT_7] = OTFC_OUTPUT_7,
#endif
      [PIN_BUTTON] = OTFC_BUTTON_PIN,
      [PIN_BUZZER] = OTFC_BUZZER_PIN,
      [PIN_LED_BLINK] = OTFC_LED_PIN,
#ifdef OTFC_SERIAL_0
      [PIN_SERIAL_0_TX] = OTFC_SERIAL_0_TX,
      [PIN_SERIAL_0_RX] = OTFC_SERIAL_0_RX,
#endif
#ifdef OTFC_SERIAL_1
      [PIN_SERIAL_1_TX] = OTFC_SERIAL_1_TX,
      [PIN_SERIAL_1_RX] = OTFC_SERIAL_1_RX,
#endif
#ifdef OTFC_SERIAL_2
      [PIN_SERIAL_2_TX] = OTFC_SERIAL_2_TX,
      [PIN_SERIAL_2_RX] = OTFC_SERIAL_2_RX,
#endif
#ifdef OTFC_I2C_0
      [PIN_I2C_0_SCL] = OTFC_I2C_0_SCL,
      [PIN_I2C_0_SDA] = OTFC_I2C_0_SDA,
#endif
#ifdef OTFC_ADC_0
      [PIN_INPUT_ADC_0] = OTFC_ADC_0_PIN,
#endif
#ifdef OTFC_ADC_1
      [PIN_INPUT_ADC_1] = OTFC_ADC_1_PIN,
#endif
#ifdef OTFC_SPI_0
      [PIN_SPI_0_SCK] = OTFC_SPI_0_SCK,
      [PIN_SPI_0_MOSI] = OTFC_SPI_0_MOSI,
      [PIN_SPI_0_MISO] = OTFC_SPI_0_MISO,
      [PIN_SPI_CS0] = OTFC_SPI_CS_GYRO,
      [PIN_SPI_CS1] = OTFC_SPI_CS_BARO,
      [PIN_SPI_CS2] = -1,
#endif
    };
    SerialPortConfig serial[SERIAL_UART_COUNT] = {
#ifdef OTFC_SERIAL_USB
      [SERIAL_USB]    = { .id = SERIAL_ID_USB_VCP, .functionMask = OTFC_SERIAL_USB_FN, .baud = SERIAL_SPEED_115200, .blackboxBaud = SERIAL_SPEED_NONE },
#endif
#ifdef OTFC_SERIAL_0
      [SERIAL_UART_0] = { .id = SERIAL_ID_UART_1, .functionMask = OTFC_SERIAL_0_FN, .baud = OTFC_SERIAL_0_BAUD, .blackboxBaud = OTFC_SERIAL_0_BBAUD },
#endif
#ifdef OTFC_SERIAL_1
      [SERIAL_UART_1] = { .id = SERIAL_ID_UART_2, .functionMask = OTFC_SERIAL_1_FN, .baud = OTFC_SERIAL_1_BAUD, .blackboxBaud = OTFC_SERIAL_1_BBAUD },
#endif
#ifdef OTFC_SERIAL_2
      [SERIAL_UART_2] = { .id = SERIAL_ID_UART_3, .functionMask = OTFC_SERIAL_2_FN, .baud = OTFC_SERIAL_2_BAUD, .blackboxBaud = OTFC_SERIAL_2_BBAUD },
#endif
#ifdef OTFC_SERIAL_SOFT_0
      [SERIAL_SOFT_0] = { .id = SERIAL_ID_SOFTSERIAL_1, .functionMask = OTFC_SERIAL_SOFT_0_FN, .baud = SERIAL_SPEED_115200, .blackboxBaud = SERIAL_SPEED_NONE },
#endif
    };
    ...
    int8_t mixerSync = 1;
    int32_t featureMask = OTFC_FEATURE_MASK;
    bool telemetry = 0;
...
// only local development settings
    ...
    void devPreset()
    {
#ifdef OTFC_DEV_PRESET_BLACKBOX_SERIAL
      blackbox.dev = BLACKBOX_DEV_SERIAL; // serial
      debug.mode = DEBUG_GYRO_SCALED;
      serial[OTFC_DEV_PRESET_BLACKBOX_SERIAL].functionMask |= SERIAL_FUNCTION_BLACKBOX;
      serial[OTFC_DEV_PRESET_BLACKBOX_SERIAL].blackboxBaud = SERIAL_SPEED_250000;
      serial[OTFC_DEV_PRESET_BLACKBOX_SERIAL].baud = SERIAL_SPEED_250000;
#endif

#ifdef OTFC_DEV_PRESET_BLACKBOX_FLASH
      blackbox.dev = BLACKBOX_DEV_FLASH; // flash
      blackbox.pDenom = 16; // 500Hz
#endif

#ifdef OTFC_DEV_PRESET_MODES
...
#ifdef OTFC_DEV_PRESET_DSHOT
      output.protocol = ESC_PROTOCOL_DSHOT300;
#endif

#ifdef OTFC_DEV_PRESET_BRUSHED
