#ifndef _OTFC_MODEL_STATE_H_
#define _OTFC_MODEL_STATE_H_
...
struct InputState
{
  ...
  // RC override via MSP  
  bool overrideActive;  
  uint32_t overrideTime;  // timestamp последнего MSP_SET_RAW_RC  
  size_t overrideCount;   // сколько каналов реально прислано в последнем MSP_SET_RAW_RC  
  float overrideUs[INPUT_CHANNELS]; // override значения в us
...
{
  Device::RangefinderDevice* dev;
  bool present;
  int32_t rate;
  uint32_t updateCount;
  int32_t raw;       // raw distance [mm], -1 when invalid
  bool valid;        // last sample was a valid in-range reading
  float distance;    // filtered slant distance [m]
  float height;      // tilt-compensated vertical distance [m]

  // external (MSP/UART, e.g. MTF-02) source feed
  int32_t extRaw;       // last raw distance from MSP [mm], negative = out of range
  uint8_t extQuality;   // signal quality [0;255]
  uint32_t extTs;       // timestamp [us] of last received MSP sample
};
...
struct FlowState
{
  bool present = false;
  bool valid = false;
  uint8_t quality = 0;   // signal quality [0;255]
  int32_t motionX = 0;   // raw motion along X from optical flow
  int32_t motionY = 0;   // raw motion along Y from optical flow
  int32_t rate = 0;      // measured update rate [Hz]
  uint32_t updateCount = 0;
  uint32_t lastMsgTs = 0; // timestamp [us] of last received MSP sample
};
...
struct AltitudeState
{
  ...
  float accelVario;
  float accelBias;
  uint32_t baroUpdateCount;
};

struct PosHoldState
{
  bool engaged = false;      // holding an anchor point
  float velX = 0.0f;         // estimated ground velocity, body X (right) [m/s]
  float velY = 0.0f;         // estimated ground velocity, body Y (forward) [m/s]
  float posX = 0.0f;         // integrated position error, body X [m]
  float posY = 0.0f;         // integrated position error, body Y [m]
  float angle[2] = {0.0f, 0.0f}; // auto lean angle setpoint [rad], index 0=roll, 1=pitch
  uint32_t lastFlowTs = 0;   // last consumed flow timestamp
  Control::Pid pidPosX;      // position -> velocity setpoint
  Control::Pid pidPosY;
  Control::Pid pidVelX;      // velocity -> lean angle
  Control::Pid pidVelY;
  Utils::Filter velFilterX;
  Utils::Filter velFilterY;
};
