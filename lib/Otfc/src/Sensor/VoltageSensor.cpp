...
int VoltageSensor::readVbat()
{
  ...
  float volts = _vFilterFast.update(_model.state.battery.rawVoltage * OTFC_ADC_SCALE);
...
int VoltageSensor::readIbat()
{
  ...
  float volts = _iFilterFast.update(_model.state.battery.rawCurrent * OTFC_ADC_SCALE);
